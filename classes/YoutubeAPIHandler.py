...
class YoutubeAPIHandler():
    def __init__(self, YOUTUBE_API_KEY):
        ...
        self.base_shortened_video_url = "youtu.be/{videoId}"
    ...
    def checkLoadListVideos(self):
        if(not self.list_videos_json):
            raise Exception("Vídeos não carregados: Use a função 'loadListVideos()' antes deste método para carregar os vídeos.")
    ...
    # RETORNA A URL DO VÍDEO COM BASE NO ID
    def getShortenedVideoURL(self, video_id):

        return self.base_shortened_video_url.format(videoId=video_id)
